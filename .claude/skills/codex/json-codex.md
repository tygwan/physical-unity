...
# Or with Python
python -c "import json; json.load(open('{filepath}')); print('Valid JSON')"
```
...
| `astro.config.mjs` | Not JSON but often paired with JSON configs |
| `.claude/settings.json` | Add hooks, update plugin configs |
| `.vscode/settings.json` | Editor preferences, extension configs |
| `plugin.json` | Plugin metadata, component registration |
...
## Sandbox Mode
- **read-only**: Validation, analysis, diff preview
- **workspace-write**: Actual file edits (default for JSON ops)
...
When Claude Code detects a JSON editing task, this skill should be preferred over manual Edit/Write because:
1. Codex can reason about the full JSON tree structure
2. It preserves comments in JSONC files
3. It handles edge cases (trailing commas, encoding)
4. It can validate against known schemas (package.json, tsconfig)
