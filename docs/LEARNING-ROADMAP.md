...
| **Phase D** | Lane Observation (254D) | ⏭️ 스킵 | (Phase E로 대체) |
| **Phase E** | 곡선 도로 (Curved Roads) | ✅ 완료 | **+931** |
| **Phase F** | 다중 차선 (Multi-Lane) | ✅ 완료 | **+988** |
| **Phase G** | 교차로 (Intersection) | 🔄 진행중 | +461 (340K) |
| **Phase H** | 신호등 + 정지선 | 📋 계획 | - |
| **Phase I** | U턴 + 특수 기동 | 📋 계획 | - |
| **Phase J** | 횡단보도 + 보행자 | 📋 계획 | - |
...
- **원인 분석**:
  - 사전학습 encoder를 unfreeze하면 기존 지식 손실
  - ONNX 형식이 ML-Agents와 호환되지 않음
  - Encoder freeze/unfreeze 전략 실패
- **교훈**: 단순 재학습이 복잡한 아키텍처보다 효과적
...
## Part 6: Visual Learning Progress (스크린샷 갤러리)

각 Phase별 학습 장면을 스크린샷으로 기록합니다.
...
저장 위치: `Assets/Screenshots/phaseX_stageY_description.png`

---

### Phase E: 곡선 도로 (Curved Roads) ✅

| Stage | 설명 | Reward | 스크린샷 |
|-------|------|--------|----------|
| Straight | 직선 도로 기본 | +400 | ![E-1](../Assets/Screenshots/phaseE_straight.png) |
| Gentle | 완만한 곡선 | +600 | ![E-2](../Assets/Screenshots/phaseE_gentle.png) |
| Moderate | 중간 곡선 | +800 | ![E-3](../Assets/Screenshots/phaseE_moderate.png) |
| Final | 최종 (곡선+NPC) | +931 | ![E-4](../Assets/Screenshots/phaseE_final.png) |

---

### Phase F: 다중 차선 (Multi-Lane) ✅

| Stage | 설명 | Reward | 스크린샷 |
|-------|------|--------|----------|
| SingleLane | 1차선 주행 | +800 | ![F-1](../Assets/Screenshots/phaseF_single.png) |
| TwoLanes | 2차선 도로 | +850 | ![F-2](../Assets/Screenshots/phaseF_two.png) |
| CenterLine | 중앙선 규칙 | +900 | ![F-3](../Assets/Screenshots/phaseF_centerline.png) |
| Final | 최종 (다차선+NPC) | +988 | ![F-4](../Assets/Screenshots/phaseF_final.png) |

---

### Phase G: 교차로 (Intersection) 🔄 진행중

| Stage | 설명 | Reward | 스크린샷 |
|-------|------|--------|----------|
| NoIntersection | 직선 도로 (워밍업) | - | 📸 캡처 예정 |
| T-Junction | T자 교차로 등장 | - | 📸 캡처 예정 |
| Cross | 십자 교차로 | - | 📸 캡처 예정 |
| Y-Junction | Y자 분기점 | - | 📸 캡처 예정 |
| LeftTurn | 좌회전 학습 | - | 📸 캡처 예정 |
| RightTurn | 우회전 학습 | - | 📸 캡처 예정 |
...
## Part 7: 학습 기록 시스템 (Training Log System)

### 성공 기록 템플릿

각 Phase 학습 완료 시 아래 템플릿을 사용하여 기록합니다.
...
#### 모델 배포
- ONNX 파일: `results/v12_phaseX/E2EDrivingAgent.onnx`
- Unity 복사: `models/planning/v12_phaseX.onnx`
```
...
|------|-----|
| **실패일** | YYYY-MM-DD |
| **Run ID** | v12_phaseX_attempt_N |
| **중단 Steps** | X,XXX,XXX |
| **최종 Reward** | -XXX |
| **실패 유형** | [수렴 실패 / 발산 / Catastrophic Forgetting / 버그] |

#### 실패 원인 분석
1. **직접 원인**: 무엇이 실패를 유발했는가?
2. **근본 원인**: 왜 그런 문제가 발생했는가?
3. **환경 요인**: 커리큘럼, 하이퍼파라미터, 코드 버그 등
...
#### 관련 파일
- 로그: `experiments/logs/v12_phaseX_attemptN.log`
- 설정: `python/configs/planning/vehicle_ppo_v12_phaseX.yaml`
```

---

### 실패 이력 (Failure History)

> ⚠️ **중요**: 이 섹션은 모든 학습 실패를 시간순으로 기록합니다.

#### 2026-01-24: v11 Sparse Reward 실패
...
**원인 분석**:
1. Sparse reward (+3.0 추월 완료 시)로는 학습 신호 부족
2. `targetSpeed = leadSpeed` 설계로 추월 동기 제거
3. `followingBonus`가 NPC 뒤 대기를 보상

**교훈**:
- Dense reward 필수 (추월 과정 전체에 보상)
- `targetSpeed`는 항상 `speedLimit`으로 고정
- 보상 구조가 의도한 행동을 유도하는지 검증 필요

---

#### 2026-01-26: v12_HybridPolicy Catastrophic Forgetting

| 항목 | 값 |
|------|-----|
| **Run ID** | v12_hybrid_policy |
| **중단 Steps** | 3,000,000 |
| **최고 Reward** | -82.7 (1.44M) |
| **최종 Reward** | -2171.9 |
| **실패 유형** | Catastrophic Forgetting |

**원인 분석**:
1. 사전학습된 encoder를 unfreeze하면서 기존 지식 손실
2. Stage 5 (모든 encoder unfreeze)에서 급격한 성능 저하
3. PyTorch → ONNX → ML-Agents 호환성 문제

**교훈**:
- Encoder freeze 전략은 ML-Agents 환경에서 비효율적
- 복잡한 아키텍처보다 처음부터 재학습이 효과적
- 전이 학습 시 호환성 먼저 검증

---

#### 2026-01-27: v12_phaseD Observation Mismatch (복구됨)

| 항목 | 값 |
|------|-----|
| **Run ID** | v12_phaseD (초기) |
| **문제** | Observation 차원 불일치 |
| **실패 유형** | 버그 |

**원인 분석**:
- Unity Space Size (254) ≠ 실제 Observation (242)
- BehaviorParameters 업데이트 누락

**해결**:
- `AddLaneObservations()` 메서드 구현
- Unity Editor에서 Space Size 254로 업데이트

---

### 성공 이력 (Success History)

> ✅ 이 섹션은 각 Phase 학습 완료를 기록합니다.
...
- `targetSpeed = speedLimit` 고정
- `stuckBehindPenalty` 도입

---

#### Phase B: Overtake vs Follow ✅
...
| 날짜 | 버전 | 변경 내용 |
|------|------|----------|
| 2026-01-27 | 1.2 | Part 7: 학습 기록 시스템 추가 (성공/실패 템플릿) |
| 2026-01-27 | 1.1 | Visual Learning Progress 섹션 추가 |
